...
function normalizeState(value) {
  ...
  const normalized = {
    ...
    accountCodes: Array.isArray(value.accountCodes) ? uniqueStrings(value.accountCodes) : [],
    accountCredentials: Array.isArray(value.accountCredentials) ? value.accountCredentials : [],
    ...
    transferRequests: Array.isArray(value.transferRequests) ? value.transferRequests : [],
    canteenCards: Array.isArray(value.canteenCards) ? value.canteenCards : [],
    canteenMealScans: Array.isArray(value.canteenMealScans) ? value.canteenMealScans : [],
...
function mergeState(existingData, incomingData) {
  return applyDeletionTombstones({
    ...
    accountCodes: uniqueStrings([...(existingData.accountCodes ?? []), ...(incomingData.accountCodes ?? [])]),
    accountCredentials: mergeRecordsById(existingData.accountCredentials ?? [], incomingData.accountCredentials ?? []),
    ...
    transferRequests: mergeRecordsById(existingData.transferRequests ?? [], incomingData.transferRequests ?? [], chooseLatestRecord),
    canteenCards: mergeRecordsById(existingData.canteenCards, incomingData.canteenCards, chooseLatestRecord),
    canteenMealScans: mergeRecordsById(existingData.canteenMealScans, incomingData.canteenMealScans, chooseLatestRecord),
