...
import { generateSchoolEmail, generateUniqueCode, isEnglishName, makeId } from '../data';
...
export function DirectorCreateAccountPanel({
  ...
  const importStudentList = (event: FormEvent<HTMLFormElement>) => {
    ...
    const invalidNames = studentNames.filter((name) => !isEnglishName(name));
    if (invalidNames.length > 0) {
      setBulkError(`${tr(language, 'englishNameRequired')} (${invalidNames.slice(0, 3).join(', ')}${invalidNames.length > 3 ? '...' : ''})`);
      setBulkCreatedCount(0);
      return;
    }
  ...
  return (
    <div className="panel">
      ...
      <form className="form-grid" onSubmit={createAccount}>
        ...
        {form.role !== 'student' ? (
          <>
            <p className="hint full">{tr(language, currentUser.stage === 'primary' ? 'createPrimaryAccountsHint' : 'createOnlyTeacherStudent')}</p>
      ...
      {form.role === 'student' && <section className="bulk-student-import">
        ...
        <form className="form-grid" onSubmit={importStudentList}>
          ...
          <p className="hint full">{tr(language, 'englishNamesListHint')}</p>
