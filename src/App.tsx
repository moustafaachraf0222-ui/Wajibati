...
type AiAccountAssignment = {
  schoolYear: number;
  stream?: SecondaryStream | '';
  classGroups: string[];
};

type AiAccountSuggestion = {
  ...
  classGroup?: string;
  stream?: SecondaryStream | '';
  assignments?: AiAccountAssignment[];
  confidence?: number;
  warnings?: string[];
  sourceText?: string;
};

type AiAccountDraftStatus = 'pending' | 'rejected' | 'created';

type AiAccountDraft = AiAccountSuggestion & {
  id: string;
  email: string;
  password: string;
  selected: boolean;
  status: AiAccountDraftStatus;
  warnings: string[];
  errors: string[];
};
...
const copy: Record<Language, Record<string, string>> = {
  ar: {
    ...
    manualCreationTab: 'يدوي',
    aiCreationTab: 'ذكاء اصطناعي',
    aiAccountImportTitle: 'إنشاء الحسابات بالذكاء الاصطناعي',
    ...
    aiImportError: 'تعذر توليد الحسابات. راجع النص أو حاول مرة أخرى.',
    aiImportEmpty: 'أدخل قائمة تحتوي على أستاذ أو تلميذ واحد على الأقل.',
    aiGeneratedAccounts: 'الحسابات المقترحة',
    aiReviewHint: 'راجع كل حساب، واقبل فقط الحسابات التي تريد إنشاءها.',
    acceptDraft: 'قبول',
    rejectDraft: 'تجاهل',
    restoreDraft: 'استرجاع',
    createAcceptedAccounts: 'إنشاء الحسابات المقبولة',
    aiNoAcceptedDrafts: 'لا توجد حسابات مقبولة جاهزة للإنشاء.',
    aiCreatedCount: 'تم إنشاء الحسابات المقبولة.',
    draftReady: 'جاهز',
    draftHasIssues: 'يحتاج مراجعة',
    draftCreated: 'تم إنشاؤه',
    draftRejected: 'متجاهل',
    teacherAssignments: 'تعيينات الأستاذ',
    generatedByAi: 'مقترح بالذكاء الاصطناعي',
    aiNoDrafts: 'لا توجد اقتراحات بعد.',
  ...
  fr: {
    ...
    manualCreationTab: 'Manuel',
    aiCreationTab: 'IA',
    aiAccountImportTitle: 'Création avec IA',
    ...
    aiImportError: 'Impossible de générer les comptes. Vérifiez le texte ou réessayez.',
    aiImportEmpty: 'Ajoutez une liste avec au moins un enseignant ou un élève.',
    aiGeneratedAccounts: 'Comptes proposés',
    aiReviewHint: 'Vérifiez chaque compte et acceptez uniquement ceux à créer.',
    acceptDraft: 'Accepter',
    rejectDraft: 'Ignorer',
    restoreDraft: 'Restaurer',
    createAcceptedAccounts: 'Créer les comptes acceptés',
    aiNoAcceptedDrafts: 'Aucun compte accepté prêt à créer.',
    aiCreatedCount: 'Comptes acceptés créés.',
    draftReady: 'Prêt',
    draftHasIssues: 'À vérifier',
    draftCreated: 'Créé',
    draftRejected: 'Ignoré',
    teacherAssignments: 'Affectations enseignant',
    generatedByAi: 'Proposé par IA',
    aiNoDrafts: 'Aucune proposition pour le moment.',
  ...
  en: {
    ...
    manualCreationTab: 'Manual',
    aiCreationTab: 'AI',
    aiAccountImportTitle: 'AI account creation',
    ...
    aiImportError: 'Could not generate accounts. Check the text or try again.',
    aiImportEmpty: 'Enter a list with at least one teacher or student.',
    aiGeneratedAccounts: 'Suggested accounts',
    aiReviewHint: 'Review each account and accept only the ones you want to create.',
    acceptDraft: 'Accept',
    rejectDraft: 'Ignore',
    restoreDraft: 'Restore',
    createAcceptedAccounts: 'Create accepted accounts',
    aiNoAcceptedDrafts: 'No accepted accounts are ready to create.',
    aiCreatedCount: 'Accepted accounts were created.',
    draftReady: 'Ready',
    draftHasIssues: 'Needs review',
    draftCreated: 'Created',
    draftRejected: 'Ignored',
    teacherAssignments: 'Teacher assignments',
    generatedByAi: 'AI suggestion',
    aiNoDrafts: 'No suggestions yet.',
...
function normalizeAiClassGroup(value: string) {
  return normalizeClassGroup(value)
...
function isSecondaryStream(value: unknown): value is SecondaryStream {
  return typeof value === 'string' && secondaryStreams.includes(value as SecondaryStream);
}
...
function uniqueEmailForSchool(
  ...
  while (usedEmails.has(candidate.toLowerCase())) {
    candidate = `${localBase}${counter}@${cleanDomain}`;
    counter += 1;
  }
...
function normalizeAiAssignments(value: unknown): AiAccountAssignment[] {
  if (!Array.isArray(value)) {
    return [];
  }

  return value
    .map((assignment): AiAccountAssignment => {
      const source = assignment && typeof assignment === 'object' ? (assignment as Record<string, unknown>) : {};
      const schoolYear = Number(source.schoolYear);
      const stream: SecondaryStream | '' = isSecondaryStream(source.stream) ? source.stream : '';
      const rawClassGroups = Array.isArray(source.classGroups)
        ? parseClassGroups(source.classGroups.map(String).join(','))
        : parseClassGroups(String(source.classGroup ?? ''));
      const classGroups = rawClassGroups.map(normalizeAiClassGroup).filter(Boolean);

      return { schoolYear, stream, classGroups };
    })
    .filter((assignment) => Number.isInteger(assignment.schoolYear) && assignment.classGroups.length > 0);
}

function normalizeAiAccountSuggestions(rawAccounts: unknown, school: SchoolRecord, data: PlatformData): AiAccountDraft[] {
  const accounts = Array.isArray(rawAccounts) ? rawAccounts : [];
  const usedEmails = new Set(data.users.map((user) => user.email.toLowerCase()));

  const drafts: AiAccountDraft[] = [];

  accounts.forEach((account, index) => {
    const source = account && typeof account === 'object' ? (account as Record<string, unknown>) : {};
    const role: 'teacher' | 'student' | null = source.role === 'teacher' || source.role === 'student' ? source.role : null;
    if (!role) {
      return;
    }
    ...
    const draft: AiAccountDraft = {
      ...
      warnings: Array.isArray(source.warnings) ? source.warnings.map(String).filter(Boolean).slice(0, 4) : [],
      sourceText: String(source.sourceText ?? '').trim(),
      selected: true,
      status: 'pending' as AiAccountDraftStatus,
      errors: []
    };
...
function mergeClassGroups(existing: string[] | undefined, nextGroups: string[]) {
  const next = [...(existing ?? [])];

  nextGroups.forEach((group) => {
    if (!next.some((existingGroup) => sameClassGroup(existingGroup, group))) {
      next.push(group);
    }
  });
...
function buildImportedAccountUser(
  ...
  if (!school || !currentUser.stage || !currentUser.schoolId) {
    errors.push(tr(language, 'lockedScope'));
  ...
  const cleanDomain = school.domain.replace(/^@/, '').trim().toLowerCase();

  if (!name) {
    errors.push(tr(language, 'fullName'));
  }

  if (!email.endsWith(`@${cleanDomain}`)) {
    errors.push(`${tr(language, 'emailMustMatchDomain')} (@${cleanDomain})`);
  }

  if (reservedEmails.has(email) || data.users.some((user) => user.email.toLowerCase() === email)) {
    errors.push(tr(language, 'duplicateEmail'));
  }

  if (!password) {
    errors.push(tr(language, 'passwordDefault'));
  }

  const maxYear = schoolYearNames[language][currentUser.stage].length;
  const isValidYear = (year: number | undefined) => Number.isInteger(year) && Number(year) >= 1 && Number(year) <= maxYear;

  if (draft.role === 'student') {
    const schoolYear = Number(draft.schoolYear);
    const classGroup = normalizeClassGroup(draft.classGroup ?? '');
    const stream = draft.stream && isSecondaryStream(draft.stream) ? draft.stream : '';

    if (!isValidYear(schoolYear)) {
      errors.push(tr(language, 'yearRequired'));
    }

    if (!classGroup) {
      errors.push(tr(language, 'classRequired'));
    }

    if (currentUser.stage === 'secondary') {
      const streamsForYear = secondaryStreamsForYear(school, schoolYear);
      if (streamsForYear.length === 0) {
        errors.push(tr(language, 'noStreamsEnabled'));
      } else if (!stream || !streamsForYear.includes(stream)) {
        errors.push(tr(language, 'streamRequired'));
      }
    }
    ...
    return {
      ...
      user: {
        ...
        schoolYear,
        classGroup,
        stream: currentUser.stage === 'secondary' && stream ? stream : undefined,
        createdBy: currentUser.id
      }
    };
  }

  const assignments = normalizeAiAssignments(draft.assignments);
  const yearClassGroups: Record<string, string[]> = {};
  const yearStreamClassGroups: YearStreamClassGroups = {};

  assignments.forEach((assignment) => {
    const year = Number(assignment.schoolYear);
    if (!isValidYear(year)) {
      warnings.push(`${tr(language, 'schoolYear')}: ${assignment.schoolYear}`);
      return;
    }

    const yearKey = String(year);
    const classGroups = parseClassGroups(assignment.classGroups.join(','));
    if (classGroups.length === 0) {
      return;
    }

    if (currentUser.stage === 'secondary') {
      const stream = assignment.stream && isSecondaryStream(assignment.stream) ? assignment.stream : '';
      const streamsForYear = secondaryStreamsForYear(school, year);
      if (!stream || !streamsForYear.includes(stream)) {
        warnings.push(`${schoolYearLabel(language, currentUser.stage, year)}: ${tr(language, 'streamRequired')}`);
        return;
      }

      const currentYear = yearStreamClassGroups[yearKey] ?? {};
      currentYear[stream] = mergeClassGroups(currentYear[stream], classGroups);
      yearStreamClassGroups[yearKey] = currentYear;
      return;
    }

    yearClassGroups[yearKey] = mergeClassGroups(yearClassGroups[yearKey], classGroups);
  });

  const teacherSchoolYears = uniqueNumbers(
    currentUser.stage === 'secondary' ? Object.keys(yearStreamClassGroups).map(Number) : Object.keys(yearClassGroups).map(Number)
  );
  const subject = isSubject(draft.subject) ? draft.subject : undefined;

  if (teacherSchoolYears.length === 0) {
    errors.push(tr(language, 'yearRequired'));
  }

  if (currentUser.stage === 'secondary' && teacherSchoolYears.some((year) => Object.keys(yearStreamClassGroups[String(year)] ?? {}).length === 0)) {
    errors.push(tr(language, 'classesRequired'));
  }

  if (currentUser.stage !== 'secondary' && teacherSchoolYears.some((year) => (yearClassGroups[String(year)] ?? []).length === 0)) {
    errors.push(tr(language, 'classesRequired'));
  }

  if (!subject) {
    errors.push(tr(language, 'subjectRequired'));
  } else {
    teacherSchoolYears.forEach((year) => {
      const options = subjectOptionsForTeacherYear(school, yearStreamClassGroups, year);
      if (!options.includes(subject)) {
        errors.push(`${schoolYearLabel(language, currentUser.stage, year)}: ${tr(language, 'subjectRequired')}`);
      }
    });
  }
  ...
  const subjectsByYear = Object.fromEntries(teacherSchoolYears.map((year) => [String(year), subject])) as Record<string, Subject>;
  const primaryYear = teacherSchoolYears[0];
  const primaryStreamGroups = yearStreamClassGroups[String(primaryYear)] ?? {};
  ...
  const primaryClassGroup =
    currentUser.stage === 'secondary'
      ...
      : yearClassGroups[String(primaryYear)]?.[0] ?? '';
  ...
  return {
    ...
    user: {
      ...
      schoolYears: teacherSchoolYears,
      yearClassGroups: currentUser.stage !== 'secondary' ? yearClassGroups : undefined,
      yearStreamClassGroups: currentUser.stage === 'secondary' ? yearStreamClassGroups : undefined,
      createdBy: currentUser.id
    }
  };
}
...
function DirectorUsersPanel({ data, setData, currentUser, language }: CommonViewProps & { setData: DataSetter }) {
  ...
  const createAccount = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!normalizedEmail.endsWith(`@${school.domain}`)) {
      setError(`${tr(language, 'emailMustMatchDomain')} (@${school.domain})`);
      return;
    }
    ...
    <section className="content-grid">
      ...
      <div className="panel">
        ...
        <form className="form-grid" onSubmit={createAccount}>
          ...
          <DirectorAiAccountImport data={data} setData={setData} currentUser={currentUser} school={school} language={language} />
        )}
      </div>
      )}
...
function DirectorAiAccountImport({
  data,
  setData,
  currentUser,
  school,
  language
}: CommonViewProps & { setData: DataSetter; school: SchoolRecord | undefined }) {
  const [rawText, setRawText] = useState('');
  const [drafts, setDrafts] = useState<AiAccountDraft[]>([]);
  ...
  const generateDrafts = async () => {
    ...
    try {
      ...
      const nextDrafts = normalizeAiAccountSuggestions(payload.accounts, school, data);
      setDrafts(nextDrafts);
      if (nextDrafts.length === 0) {
        setError(tr(language, 'aiImportEmpty'));
      }
    } catch {
      setError(tr(language, 'aiImportError'));
    } finally {
      setIsGenerating(false);
    }
  };

  const updateDraft = (draftId: string, update: Partial<AiAccountDraft>) => {
    setDrafts((previous) => previous.map((draft) => (draft.id === draftId ? { ...draft, ...update } : draft)));
  };

  const createAcceptedDrafts = () => {
    if (!school || !currentUser.stage || !currentUser.schoolId) {
      return;
    }

    const reservedEmails = new Set(data.users.map((user) => user.email.toLowerCase()));
    const usersToCreate: PlatformUser[] = [];
    let hasSelectedDraft = false;

    const nextDrafts = drafts.map((draft) => {
      if (!draft.selected || draft.status !== 'pending') {
        return draft;
      }

      hasSelectedDraft = true;
      const result = buildImportedAccountUser(draft, data, currentUser, school, language, reservedEmails);

      if (!result.user) {
        return {
          ...draft,
          errors: result.errors,
          warnings: [...new Set([...draft.warnings, ...result.warnings])]
        };
      }

      reservedEmails.add(result.user.email.toLowerCase());
      usersToCreate.push(result.user);

      return {
        ...draft,
        selected: false,
        status: 'created' as AiAccountDraftStatus,
        errors: [],
        warnings: [...new Set([...draft.warnings, ...result.warnings])]
      };
    });

    if (!hasSelectedDraft || usersToCreate.length === 0) {
      setDrafts(nextDrafts);
      setError(hasSelectedDraft ? tr(language, 'draftHasIssues') : tr(language, 'aiNoAcceptedDrafts'));
      setMessage('');
      return;
    }
    ...
    setData((previous) => ({
      ...previous,
      users: [...previous.users, ...usersToCreate]
    }));
    setDrafts(nextDrafts);
    setError('');
    setMessage(`${tr(language, 'aiCreatedCount')} (${usersToCreate.length})`);
  };

  const draftStatusLabel = (draft: AiAccountDraft, errors: string[]) => {
    if (draft.status === 'created') {
      return tr(language, 'draftCreated');
    }

    if (draft.status === 'rejected') {
      return tr(language, 'draftRejected');
    }

    return errors.length > 0 ? tr(language, 'draftHasIssues') : tr(language, 'draftReady');
  };

  const draftStatusClass = (draft: AiAccountDraft, errors: string[]) => {
    if (draft.status === 'created') {
      return 'created';
    }
    ...
    return errors.length > 0 ? 'warning' : 'ready';
  };

  const assignmentLabels = (draft: AiAccountDraft) => {
    if (draft.role === 'student') {
      const parts = [
        schoolYearLabel(language, currentUser.stage, draft.schoolYear),
        currentUser.stage === 'secondary' && draft.stream && isSecondaryStream(draft.stream) ? secondaryStreamLabel(language, draft.stream, draft.schoolYear) : '',
        draft.classGroup ? `${tr(language, 'classGroup')} ${draft.classGroup}` : ''
      ].filter((part) => part && part !== '-');

      return parts.length > 0 ? parts : [tr(language, 'noAssignments')];
    }
    ...
    return assignments.map((assignment) => {
      const parts = [
        schoolYearLabel(language, currentUser.stage, assignment.schoolYear),
        currentUser.stage === 'secondary' && assignment.stream && isSecondaryStream(assignment.stream)
          ? secondaryStreamLabel(language, assignment.stream, assignment.schoolYear)
          : '',
        assignment.classGroups.length > 0 ? `${tr(language, 'classGroups')} ${assignment.classGroups.join('، ')}` : ''
      ].filter((part) => part && part !== '-');
  ...
  return (
    <div className="ai-import-panel">
      <div className="ai-import-copy">
        <Sparkles size={22} aria-hidden="true" />
        <div>
          <h3>{tr(language, 'aiAccountImportTitle')}</h3>
          <p>{tr(language, 'aiAccountImportText')}</p>
        </div>
      </div>
      <label className="ai-input-block">
        <span>{tr(language, 'aiInputLabel')}</span>
        <textarea
          value={rawText}
          rows={7}
          placeholder={tr(language, 'aiInputPlaceholder')}
          disabled={isGenerating || isLocalFile}
      ...
      <div className="button-row">
        <button className="button primary" type="button" disabled={isGenerating || isLocalFile} onClick={generateDrafts}>
          ...
          <span>{isGenerating ? tr(language, 'aiGenerating') : tr(language, 'generateAccountsAi')}</span>
        </button>
        <button className="button ghost" type="button" disabled={isGenerating || drafts.length === 0} onClick={() => setDrafts([])}>
          <X size={17} aria-hidden="true" />
          <span>{tr(language, 'cancel')}</span>
        </button>
      </div>
      ...
      <section className="ai-drafts">
        <div className="ai-drafts-heading">
          <div>
            <h3>{tr(language, 'aiGeneratedAccounts')}</h3>
            <p>{tr(language, 'aiReviewHint')}</p>
          </div>
          <strong>{drafts.length}</strong>
        </div>
        {drafts.length === 0 && <p className="empty-state">{tr(language, 'aiNoDrafts')}</p>}
        {drafts.map((draft) => {
          const validation = draft.status === 'pending' ? validateDraftForDisplay(draft) : { errors: [], warnings: [] };
          const errors = draft.errors.length > 0 ? draft.errors : validation.errors;
          const warnings = [...new Set([...draft.warnings, ...validation.warnings])];
          const RoleIcon = roleIcons[draft.role];
          const isPending = draft.status === 'pending';

          return (
            <article className={`ai-draft-card ${draftStatusClass(draft, errors)}`} key={draft.id}>
              <div className="ai-draft-topline">
                <div className="ai-draft-role">
                  <RoleIcon size={18} aria-hidden="true" />
                  <div>
                    <strong>{draft.name || '-'}</strong>
                    <span>{roleNames[language][draft.role]}</span>
                  </div>
                </div>
                <span className={`ai-draft-state ${draftStatusClass(draft, errors)}`}>{draftStatusLabel(draft, errors)}</span>
              </div>
              <div className="ai-draft-fields">
                ...
                <div>
                  <span>{tr(language, 'passwordDefault')}</span>
                  <strong>{draft.password}</strong>
                </div>
                {draft.role === 'teacher' && (
                  <div>
                    <span>{tr(language, 'subject')}</span>
                    <strong>{draft.subject && isSubject(draft.subject) ? subjectNames[language][draft.subject] : '-'}</strong>
                  </div>
                )}
              </div>
              <div className="ai-assignment-list">
                <span>{draft.role === 'teacher' ? tr(language, 'teacherAssignments') : tr(language, 'assignments')}</span>
                <div>
                  {assignmentLabels(draft).map((label) => (
                    <span className="assignment-chip" key={label}>
                      {label}
                    </span>
                  ))}
                </div>
              ...
              {(warnings.length > 0 || errors.length > 0) && (
                <div className="ai-draft-notes">
                  {errors.map((item) => (
                    <p className="form-error" key={`error-${item}`}>{item}</p>
                  ))}
                  {warnings.map((item) => (
                    <p className="hint" key={`warning-${item}`}>{item}</p>
                  ))}
                </div>
              )}
              <div className="button-row">
                {isPending ? (
                  <>
                    <button
                      className={draft.selected ? 'button success' : 'button ghost'}
                      type="button"
                      onClick={() => updateDraft(draft.id, { selected: !draft.selected })}
                    >
                      <CheckCircle2 size={17} aria-hidden="true" />
                      <span>{tr(language, 'acceptDraft')}</span>
                    </button>
                    <button className="button ghost" type="button" onClick={() => updateDraft(draft.id, { selected: false, status: 'rejected' })}>
                      <X size={17} aria-hidden="true" />
                      <span>{tr(language, 'rejectDraft')}</span>
                    </button>
                  </>
                ) : draft.status === 'rejected' ? (
                  <button className="button ghost" type="button" onClick={() => updateDraft(draft.id, { selected: true, status: 'pending' })}>
                    <CheckCircle2 size={17} aria-hidden="true" />
                    <span>{tr(language, 'restoreDraft')}</span>
                  </button>
                ) : (
                  <span className="hint">{tr(language, 'generatedByAi')}</span>
                )}
              </div>
            </article>
          );
      ...
      {drafts.length > 0 && (
        <button className="button primary wide" type="button" onClick={createAcceptedDrafts}>
          <Plus size={17} aria-hidden="true" />
          <span>{tr(language, 'createAcceptedAccounts')}</span>
        </button>
      )}
    </div>
  );
